...
- **Joint 5%+ filings: `prior_ownership_pct`, `ownership_pct`, and `shares_held` are now read from the group-total context.** A joint Doc 350 filing carries each co-reporter's own figures under `...FilerLargeVolumeHolder<N>Member` and the 合計 under the bare `FilingDateInstant` context. The parser selected these fields by document position: `ownership_pct` and `shares_held` took the last match (the total, by accident of row order), `prior_ownership_pct` took the first (holder 1's own prior). The result was a group total compared against one holder's prior — 375 of 400 joint filings sampled since 2024 stored a prior that disagreed with the filed total-context prior, and `ownership_change` inherited the error on every joint row (光通信 on 4491: group 18.69% → 13.09%, a sale, reported as +6.4 pts). Selection is now explicit: the bare total context, else the primary holder's row (the only row a single-filer filing carries), else positional first-match for legacy un-axised filings. Per-holder figures are unchanged on `joint_holders`.
- **A second co-reporter axis, `...JointHolder<N>Member`, is now recognized.** Some filers report 共同保有者 under this axis rather than as `FilerLargeVolumeHolder<N>` (S100SKDY: 三菱商事 as Holder1 with UCC entities as JointHolder1/2). Filings that carry it were flagged `is_joint_filing = False` with `joint_holder_count = 1`, so the group total was read as a single holder's stake. `is_joint_filing` now considers both axes and `joint_holders` lists co-reporters from both, renumbered densely with the primary filer first. About 3% of filings previously flagged single-filer carry this axis.
