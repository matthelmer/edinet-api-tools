...
- **Joint 5%+ filings are read at group grain.** When several parties file one large-holding report together, the XBRL carries each co-reporter's figures under a per-holder context (`…FilerLargeVolumeHolder<N>Member`) and the group's combined figures under the bare `FilingDateInstant` context. The parser picked among those rows by position: `ownership_pct` and `shares_held` took the last match, which happened to be the total, and `prior_ownership_pct` took the first, which was holder 1's own prior. So `prior_ownership_pct` and `ownership_change` on every joint filing compared a group total against one holder's history. 光通信's 2026-09-03 change report on コンピューターマネージメント files a group stake of 13.09% against a group prior of 14.09%, a sale of one point; 0.8.3 reported a prior of 6.70% and a gain of 6.4 points. In a sample of 400 joint filings since 2024, 375 stored a prior that disagreed with the filed group prior. Selection is now by context, not position: the un-dimensioned row first, then the primary holder's row (the only row a single-filer filing carries), then positional first-match for legacy filings with neither. A filed-but-empty total is an empty total, not a licence to report one holder's stake as the group's. Per-holder figures are unchanged on `joint_holders`. Two real filings pin this: the 光通信 report above and S100SKDY below.
- **A second co-reporter axis, `…JointHolder<N>Member`, is recognised.** Some filers report 共同保有者 under this axis rather than as `FilerLargeVolumeHolder<N>`. S100SKDY (三菱商事 with two UCC entities on ユニカフェ) carries 三菱商事 as `Holder1` at 9.50% and the UCC entities as `JointHolder1` (50.53%) and `JointHolder2` (0%), group 60.04%. 0.8.3 saw one holder: `is_joint_filing = False`, `joint_holder_count = 1`, prior 9.50%, change +50.5 points on what was an intra-group transfer. Both axes now mark a joint filing and both feed `joint_holders`. About 3% of filings previously flagged single-filer carry this axis.
- **`important_proposal` is read across co-reporters.** The field is per-holder with no group row; holder 1's `－` no longer hides a co-reporter's stated act. The first holder that states one wins; when every holder is blank, holder 1's blank marker is returned as filed. `purpose`, also per-holder, is now the primary filer's by context rather than by row order (in 2 of 600 sampled joint filings a co-reporter's purpose row came first). Co-reporters' purposes are on `joint_holders`.
- **A thousands separator in a per-share figure no longer aborts the annual-report parse.** `net_assets_per_share`, `earnings_per_share`, and the three IFRS summary per-share fields were converted with a bare `Decimal()`; `1,234.5` raised out of `parse_securities_report`, taking every other field with it. They now go through the new `parse_decimal`, which returns None for anything `Decimal()` rejects — the guard the quarterly parser already had.
- **`Entity.documents()` and `get_documents_for_date_range()` no longer swallow a rejected API key.** Both caught every exception per day and continued, so an `AuthenticationError` over a 30-day window came back as `[]` — the same silent-empty answer 0.8.1 removed from the fetchers, one layer up. `AuthenticationError` now propagates at once. A transient failure on one day is still tolerated, but a window in which every day failed raises `APIError` instead of returning an empty list.
- **The API key is resolved when a request is built, not when the package is imported.** `config.EDINET_API_KEY` was read once at import and used as the fetchers' fallback, so a key set afterwards (a notebook, `load_dotenv()` after the import) went out as the literal string `None`, and `configure(api_key=…)` never reached `api.fetch_document`. Both fetchers now resolve the key per call — explicit argument, then `configure()`, then the environment — and raise `AuthenticationError` before contacting EDINET when none is available. `config.EDINET_API_KEY` remains as an import-time snapshot for callers that read it.
- **`DOCUMENT_TYPES` and the `filter_documents` gate come from the document-type registry.** `config.SUPPORTED_DOC_TYPES` was a second, hand-maintained table that had drifted to 39 codes: 290, 310, and 330 (tender-offer opinion, Q&A, exemption) were missing while their amendment codes were present, and 370 and 380 carried the wrong names. `filter_documents` gated on it, so a date-range harvest silently dropped those three types. Both now derive from `doc_types`, and a test holds the three registries equal at 42 codes.
- **The code-list loader downloads what the FSA serves.** `EdinetDataLoader.download_edinet_codes` streamed `disclosure2.edinet-fsa.go.jp/weee0020/EDINET_Code_List.csv`, which now redirects to an HTML page; the loader wrote that page to `edinet_codes.csv` and reported success. It now fetches `Edinetcode.zip` from `disclosure2dl.edinet-fsa.go.jp`, unpacks the single CSV member, writes it atomically, and refuses anything that is not a zip with exactly one non-empty CSV — a failed refresh leaves the previous file untouched. `EDINET_CODES_ZIP_URL` carries the address; `EDINET_CSV_URL` is removed; `EDINET_CODES_URL` now points at the EDINET site for a manual download.
- Importing the package no longer writes a warning to the root logger when the key is unset. A missing key surfaces as `AuthenticationError` at the point of use; parsing and entity lookup need no key.
- Raised `HTTPError`s no longer carry the API key in their URL.
- `AuthenticationError`'s default message points at the current API-key page.

### Changed

- **`JointHolder.holder_number` is a dense `1..K` ordering on every filing** — the primary filer first, then `FilerLargeVolumeHolder` members by N, then `JointHolder` members by N. It is no longer the filed axis index: a gapped axis (1, 3) is emitted as (1, 2). A holder now exists as soon as its axis appears in a context id, so `joint_holder_count` always agrees with `is_joint_filing`.
- New public helper `edinet_tools.parsers.extraction.parse_decimal`.
