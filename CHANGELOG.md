...
- **`EntityType.FUND` renamed to `EntityType.FUND_ISSUER`** (value `"fund"` → `"fund_issuer"`), and `EntityClassifier.is_fund()` to `is_fund_issuer()`. The classification is built from the fund registry's *issuer* column — membership means "this entity has issued fund products", not "this entity is a fund". Trust banks routinely appear as issuers of beneficiary certificates, and a single issuer code can back hundreds of fund rows, so the old name invited misreads. The rename makes the API say what the data actually is.

### Added

- **`StaleDataWarning` on old code lists.** Listing status and fund-issuer membership drift as companies delist, IPO, and register funds — the bundled FSA CSVs are a snapshot of their download date, exposed via `EntityClassifier.data_version`. The classifier now warns when a loaded code list is older than `STALE_DATA_WARNING_DAYS` (365) and points at the `edinet_codes_path` / `fund_codes_path` escape hatch; undated files skip the check rather than guess. Bundled code lists refreshed to 2026-08-08 (11,376 entities / 6,369 fund rows), and the `EntityType` docstring now spells out the snapshot semantics: the registry flips a delisted company to 非上場 and blanks its securities code, and newly-listed companies can lag the registry by weeks. A release-gate test fails when the bundled data ages past the warning threshold.
