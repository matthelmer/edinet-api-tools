...
- **Joint 5% filings: the "previous stake" and "change" fields were wrong.** A joint filing is one large-holding report filed by several parties together. Every number in the XBRL carries a label saying whose it is: one label per party, and one for the group total. The parser ignored the labels and picked numbers by their position in the file. The current stake happened to land on the group total. The previous stake landed on the first party's own number. So the change figure was the group's stake today minus one party's stake last time. Example: 光通信 and a partner reported 13.09% of コンピューターマネージメント on 2026-09-03, down from 14.09%. They were selling. Version 0.8.3 reported a previous stake of 6.70% and a gain of 6.4 points. In a sample of 400 joint filings since 2024, 375 had this problem. The parser now reads the labels. It takes the group-total row when there is one. On a single-filer report there is no total row, so it takes the lead filer's row. Only a very old filing with neither falls back to the first row in the file. If the total row exists but is empty, the result is empty; it never substitutes one party's number. Each party's own figures are still on `joint_holders`. Two real filings are now test fixtures.
- **Some joint filings were treated as single-filer.** Filers have two ways to label their partners in the XBRL. The parser knew only one of them (`FilerLargeVolumeHolder<N>Member`) and not the other (`JointHolder<N>Member`). Example: 三菱商事 filed with two UCC companies on ユニカフェ. 三菱商事 alone held 9.50%; the group held 60.04%. Version 0.8.3 saw one holder at 9.50% and then reported a 50-point jump. Both labels now count as a joint filing, and both feed `joint_holders`. About 3% of filings marked single-filer had this problem.
- **The "important proposal" field now looks at every party on a joint filing.** It has no group row. Before, if the lead filer left it blank, a partner's stated proposal was hidden. Now the first party that states one wins. If all are blank, the lead filer's blank marker is returned as filed. The `purpose` field is the lead filer's, chosen by its label rather than by position. In 2 of 600 sampled joint filings a partner's purpose row came first in the file.
- **A comma in a per-share number no longer crashes the annual-report parser.** Five per-share fields were turned into numbers with no error handling. A value like `1,234.5` raised an error and lost every other field in the report. They now use the new `parse_decimal`, which returns None for anything it cannot read. The quarterly parser already worked this way.
- **A rejected API key no longer looks like an empty result.** `Entity.documents()` and `get_documents_for_date_range()` caught every error, day by day, and kept going. A bad key over a 30-day window returned an empty list. Version 0.8.1 fixed this in the low-level fetchers; these two paths sit above them and still had it. `AuthenticationError` now raises at once. One failed day is still tolerated. If every day in the window fails, `APIError` raises instead of returning nothing.
- **The API key is read when a request is made, not when the package is imported.** Before, the key was read once at import and reused. If you set the key later, for example in a notebook or with `load_dotenv()` after the import, the request went out with the word `None` as the key. `configure(api_key=...)` did not reach `api.fetch_document` either. Both fetchers now look up the key on each call: the argument first, then `configure()`, then the environment. With no key at all they raise `AuthenticationError` before contacting EDINET. `config.EDINET_API_KEY` still exists as a snapshot for code that reads it.
- **`filter_documents` silently dropped three document types.** The package had two copies of the document-type table, and the copy in `config` was out of date: 39 entries instead of 42. Codes 290, 310 and 330 (tender-offer opinion, Q&A, exemption) were missing. Codes 370 and 380 had the wrong names. `DOCUMENT_TYPES` and the filter now both come from `doc_types`, and a test keeps them equal.
- **The code-list downloader was fetching a dead URL.** `EdinetDataLoader.download_edinet_codes` read a CSV address on `disclosure2.edinet-fsa.go.jp` that now redirects to an HTML page. It saved that page as `edinet_codes.csv` and reported success. It now downloads `Edinetcode.zip` from `disclosure2dl.edinet-fsa.go.jp`, unpacks the one CSV inside, and writes it in a single step. Anything that is not a zip with exactly one non-empty CSV is refused, and the old file is kept. `EDINET_CODES_ZIP_URL` is the new address. `EDINET_CSV_URL` is removed. `EDINET_CODES_URL` now points at the EDINET site for a manual download.
- Importing the package no longer writes a warning to the root logger when no key is set. A missing key raises `AuthenticationError` when you try to use it. Parsing and entity lookup need no key.
- Raised `HTTPError`s no longer include the API key in the URL.
- `AuthenticationError` points at the current API-key page.

### Changed

- **`JointHolder.holder_number` now counts 1, 2, 3, ... with no gaps, on every filing.** The lead filer is 1, then the `FilerLargeVolumeHolder` parties in order, then the `JointHolder` parties in order. It is no longer the number from the XBRL label. A filing whose labels say 1 and 3 now gives 1 and 2. A party is counted as soon as its label appears, so `joint_holder_count` always matches `is_joint_filing`.
