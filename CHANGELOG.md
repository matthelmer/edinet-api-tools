...
- **Joint 5%+ filings: `prior_ownership_pct`, `ownership_pct`, and `shares_held` are now read from the group-total context.** A joint Doc 350 filing carries each co-reporter's own figures under `...FilerLargeVolumeHolder<N>Member` and the 合計 under the bare `FilingDateInstant` context. The parser selected these fields by document position: `ownership_pct` and `shares_held` took the last match (the total, by accident of row order), `prior_ownership_pct` took the first (holder 1's own prior). The result was a group total compared against one holder's prior — 375 of 400 joint filings sampled since 2024 stored a prior that disagreed with the filed total-context prior, and `ownership_change` — computed from that prior — was wrong wherever the prior was (光通信 on 4491, 2026-09-03 filing: group 13.09% against a filed group prior of 14.09%, a sale of 1.0 pt, reported as +6.4 pts against holder 1's own 6.70%). Selection is now explicit: the bare total context, else the primary holder's row (the only row a single-filer filing carries), else positional first-match for legacy un-axised filings. Per-holder figures are unchanged on `joint_holders`.
- **A second co-reporter axis, `...JointHolder<N>Member`, is now recognized.** Some filers report 共同保有者 under this axis rather than as `FilerLargeVolumeHolder<N>` (S100SKDY: 三菱商事 as Holder1 with UCC entities as JointHolder1/2). Filings that carry it were flagged `is_joint_filing = False` with `joint_holder_count = 1`, so the group total was read as a single holder's stake. `is_joint_filing` now considers both axes and `joint_holders` lists co-reporters from both. `holder_number` is now a dense 1..K ordering (primary filer first, then `FilerLargeVolumeHolder` members by N, then `JointHolder` members by N) on every filing — a gapped single-axis filing (1, 3) is emitted as (1, 2), where 0.8.3 emitted the filed N. A holder now exists as soon as its axis appears in a context id, so `joint_holder_count` always agrees with `is_joint_filing`. About 3% of filings previously flagged single-filer carry the second axis.
- **`important_proposal` is read across co-reporters.** The intent field is per-holder with no group row; the first holder that states an act now wins, so holder 1's `－` no longer hides a co-reporter's stated proposal (~2% of joint filings differ by holder). When every holder is blank, holder 1's blank marker is returned as filed. `purpose` stays the primary filer's, with co-reporters' on `joint_holders`.
- **The EDINET code-list loader downloads the zip the FSA actually serves.** `EdinetDataLoader.download_edinet_codes` streamed `disclosure2.edinet-fsa.go.jp/weee0020/EDINET_Code_List.csv`, which now redirects to an HTML page; the loader wrote that page into `edinet_codes.csv` and reported success. It now fetches `Edinetcode.zip` from `disclosure2dl.edinet-fsa.go.jp`, unpacks the single CSV member, and refuses to write anything that is not a zip with exactly one CSV. `EDINET_CODES_ZIP_URL` carries the address; `EDINET_CSV_URL` is removed.
- `AuthenticationError`'s default message points at the current API-key page (`api.edinet-fsa.go.jp/api/auth/index.aspx?mode=1`).
- **A thousands separator in a per-share figure no longer aborts the annual-report parse.** `net_assets_per_share`, `earnings_per_share`, and the three IFRS summary per-share fields were converted with a bare `Decimal()`; a value like `1,234.5` raised out of `parse_securities_report`. They now go through `parse_decimal`, which returns None for anything `Decimal()` rejects — the guard the quarterly parser already had. New public helper `edinet_tools.parsers.extraction.parse_decimal`.
- **`Entity.documents()` and `get_documents_for_date_range()` no longer swallow a rejected API key.** Both caught every exception per day and continued, so an `AuthenticationError` over a 30-day window returned `[]` — the silent-empty class 0.8.1 closed at the fetcher layer, still open one layer up. `AuthenticationError` now propagates at once; transient per-day failures are still tolerated, but a range where every day failed raises `APIError` instead of returning an empty list.
- **The API key is resolved when a request is built, not at import.** `config.EDINET_API_KEY` was read once at import and used as the fetchers' fallback, so a key set afterwards (notebooks, `load_dotenv()` after import) was sent as the literal string `None`, and `configure(api_key=...)` never reached `api.fetch_document`. Both fetchers now resolve the key per call — explicit argument, then `configure()`, then the environment — and raise `AuthenticationError` before making any request when none is set. `config.EDINET_API_KEY` remains as an import-time snapshot for callers that read it.
- **`DOCUMENT_TYPES` and the `filter_documents` gate are derived from the document-type registry.** `config.SUPPORTED_DOC_TYPES` was a second hand-maintained table that had drifted to 39 codes: 290 / 310 / 330 (tender-offer opinion, Q&A, exemption) were missing while their amendment codes were present, and 370 / 380 were mislabelled. `filter_documents` gated on it, so a date-range harvest silently dropped those three types. Both now derive from `doc_types`; a test pins the three registries equal (42 codes).
- **Importing the package no longer writes to the root logger.** `config.py` called `logging.warning` at import when the key was unset, which configures and pollutes the host application's logging even on offline paths that need no key.
- **The API key is masked in raised `HTTPError` URLs.** Tracebacks and bug reports no longer carry `Subscription-Key=<secret>`.
