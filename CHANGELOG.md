...
- **Full-corpus cross-field identity check (independent of the equivalence proof above).** `equity_ratio` computed from each filing's own `total_assets` and owners-equity operands, compared against the filer's own stated ratio, tolerance ±0.005: J-GAAP 30,338 of 30,340 evaluable filings agree (99.99%; the 2 exceptions are known filer-side reporting outliers, not extraction defects), US-GAAP 77 of 77. This is the check that surfaced the highlights-table defect above for IFRS; a clean post-fix IFRS agreement figure requires its own full-corpus re-run, not complete as of this writing — no IFRS rate is published until it lands clean.
...
- **Test count: 908 → 1030 across the stage-5 migration work** — new golden-fixture panels for the highlights-table fix, the semi-annual context/standard fix, the tender-results remap, and the `ProfitLossIFRS` last-resort tier (all above), plus the tier-table migration's own behavior-pin tests.
